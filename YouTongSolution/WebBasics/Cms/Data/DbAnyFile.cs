...
namespace WebBasics.Cms.Data
{
    ...
    public class DbAnyFile
    {
        ...
        {
            var where = AnyFile._.ID.In(ids) && AnyFile._.Recommend != recommend;

            return dbSession.Update<AnyFile>(
                new Field[] { AnyFile._.Recommend, AnyFile._.UpdateTime },
        ...
        public IList<AnyFile> GetAnyFiles(Guid channelId, string text, int pageIndex, int pageSize, out int rowTotal)
        {
            ...
            else
            {
                ...
                var ids = DbChannel.Instance.GetChildChannelIDs(channelId, true);
                ids.Add(channelId);
                where = where && AnyFile._.ChannelID.In(ids.ToArray()) && AnyFile._.Title.Like(string.Format("%{0}%", text));

                PageSection<AnyFile> ps = dbSession.From<AnyFile>()
                   .Where(where).OrderBy(AnyFile._.AddTime.Desc)
                   .GetPage(pageSize);
                rowTotal = ps.RowCount;
